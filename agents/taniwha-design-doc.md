...
You have been given a task in your prompt. Your job is to produce a design document at the path the dispatcher specified, then exit. The path will typically be under `.taniwha/orchestrator/handoff/<id>/outputs/`.

You have read access to whatever input documents the dispatcher placed in `.taniwha/orchestrator/handoff/<id>/inputs/`. You do not have access to other parts of the project — work from your inputs only.
